...
1) list_muscle_groups 2) уточни предпочтения (или list_exercises для релевантных групп) 3) только потом собирай структуру 4) получи историю выполнения упражнения (конкретного или аналогичных) чтобы выбрать количество подходов и вес (если есть данные).
